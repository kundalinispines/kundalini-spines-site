...
- **Static HTML/CSS/JS, no build step.** Serve with `python -m http.server 8000` — the carousel and Transmissions use `fetch()`, which browsers block on `file://`. The V2 prototype is `spine-lab.html`.
